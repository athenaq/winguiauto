...
def dbclick(hwnd):
    ...
    :return:
    '''
    win32gui.PostMessage(hwnd, win32con.WM_LBUTTONDBLCLK, None, None)
    time.sleep(.2)
...
def getEditText(hwnd):
    bufLen = win32gui.SendMessage(hwnd, win32con.WM_GETTEXTLENGTH) + 1
    buff = win32gui.PyMakeBuffer(bufLen)
    win32gui.SendMessage(hwnd, win32con.WM_GETTEXT, bufLen, buff)
    text = buff[:bufLen-1]
    return text
